...
async function launch() {
  try {
    ...
    if (!fs.existsSync(session + "/creds.json")) {
        ...
        await sleep(1000);
        const opsi = process.argv[2]; // Ambil opsi dari argumen command line
        const nomor = process.argv[3]; // Ambil nomor dari argumen command line

        if (opsi === "pairing" && nomor) {
            global.pairingCode = true;
            const phoneNumber = nomor.replace(/[+ -]/g, "");
            let code = await Exp.requestPairingCode(phoneNumber);
